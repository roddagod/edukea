...
import { fmtDate, fmtSex, fmtBool } from '../shared/formatters';
import type { SchoolInfo, StudentRef, YearRef } from '../types';

export interface ClassRollCallData {
  school: SchoolInfo;
  year: YearRef;
...
export function ClassRollCallDocument({ data }: { data: ClassRollCallData }) {
  const { school, year, classroomName, students } = data;

  return (
    <Document title={`Liste d'appel – ${classroomName}`} author={school.name}>
      <Page size="A4" orientation="landscape" style={s.pageLandscape}>
        <SchoolHeader
          school={school}
          year={year}
          title={`LISTE D'APPEL : ${classroomName}`}
          rightBlock={
        ...
        <View style={s.table}>
          ...
          <View style={s.tHead}>
            ...
            <Text style={[s.th, { flex: 2.5 }]}>Nom & Prénoms</Text>
            <Text style={[s.th, { width: 22 }]}>Sexe</Text>
            <Text style={[s.th, { width: 58 }]}>Né(s) le</Text>
            <Text style={[s.th, { flex: 1 }]}>Nationalité</Text>
            <Text style={[s.th, { flex: 1.2 }]}>Lieu de naissance</Text>
            <Text style={[s.th, { width: 44 }]}>Redoublant</Text>
            <Text style={[s.th, { width: 44 }]}>Type</Text>
            <Text style={[s.th, { width: 36 }]}>LV2</Text>
            <Text style={[s.th, { flex: 1 }]}>Matière sec.</Text>
            <Text style={[s.th, { width: 62, borderRightWidth: 0 }]}>Date d'inscription</Text>
          </View>
          ...
          {students.map((stu, i) => (
            <View key={i} style={i === students.length - 1 ? s.tRowLast : s.tRow}>
              ...
              <Text style={[s.td, { flex: 1.2 }]}>{stu.birthPlace ?? '—'}</Text>
              <Text style={[s.td, { width: 44 }]}>{fmtBool(stu.isRepeat)}</Text>
              <Text style={[s.td, { width: 44 }]}>{stu.studentTypeLabel ?? '—'}</Text>
              <Text style={[s.td, { width: 36 }]}>{stu.lv2 ?? '—'}</Text>
              <Text style={[s.td, { flex: 1 }]}>{stu.secondarySubject ?? '—'}</Text>
              <Text style={[s.td, { width: 62, borderRightWidth: 0 }]}>{fmtDate(stu.enrollmentDate)}</Text>
            </View>
          ))}
        </View>
...
export default ClassRollCallDocument;
